...
export default function DefaultLayout({ children }: React.PropsWithChildren<unknown>) {
  ...
  return (
    <div className="flex h-screen w-full bg-background text-foreground overflow-hidden font-sans">
      ...
      <div className="hidden lg:flex w-20 shrink-0 border-r border-border/40 bg-background z-20">
        <Sidebar />
      </div>
        ...
        <header className="lg:hidden flex items-center justify-between p-4 shrink-0 border-b border-border/40 bg-background/80 backdrop-blur-md z-30">
          ...
          <Sheet open={isMobileOpen} onOpenChange={setIsMobileOpen}>
            <SheetTrigger asChild>
              <Button variant="ghost" size="icon" className="text-foreground hover:bg-accent rounded-xl h-10 w-10">
                <Menu className="h-5 w-5" aria-hidden="true" />
                <span className="sr-only">Toggle mobile menu</span>
              </Button>
            </SheetTrigger>
            <SheetContent side="left" className="p-0 w-20 border-none bg-background shadow-2xl">
